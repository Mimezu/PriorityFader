...
- Adds **Stealthed / invisible**, **Fishing**, and **In Delve** conditions.
  Stealth refreshes only on player aura changes; fishing uses the playerâ€™s
...
- Added a reusable, non-destructive guided tutorial for the complete
  Manage â†’ React â†’ Order â†’ Otherwise â†’ Preview flow, followed by a clear handoff
...
- Adds a compact Managed only toggle to the target rail. It shows only fram÷­m¢G§²ÚîÆ­yÒ peek
...
- Fixed the frame pickerâ€™s rectangle handling, which could repeatedly error on
...
- Replaced the old cycling fade button with a compact timing panel for each
  target: transition time (0.05â€“2.00s) and fade-out delay (0â€“5.00s).
...
## 0.2.0 â€” rule palette and safe state snapshot
